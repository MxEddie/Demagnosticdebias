...
class TextDataset(Dataset):
    def __init__(self, tokenizer: PreTrainedTokenizer, args, file_path: str, block_size=512):
        ...
        if os.path.exists(cached_features_file) and not args.overwrite_cache:
            ...
            tokenized_text = tokenizer.convert_tokens_to_ids(tokenizer.tokenize(text,truncation=True,max_length=512))
...
def load_and_cache_examples(data, args, tokenizer):
    if args.line_by_line:
        ...
        return TextDataset(tokenizer, args, file_path=args.data_file, block_size=args.block_size)
...
def split_data(attributes_examples, attributes_labels, neutral_examples, neutral_labels, args):
    ...
    for i, (examples, labels) in enumerate(zip(attributes_examples, attributes_labels)):
        ...
        data['train']['example'][f'attribute{i}'] = examples[args.dev_data_size:10000]
        data['train']['label'][f'attribute{i}'] = labels[args.dev_data_size:10000]
        data['dev']['example'][f'attribute{i}'] = examples[:args.dev_data_size]
        data['dev']['label'][f'attribute{i}'] = labels[:args.dev_data_size]

    for i, (examples, labels) in enumerate(zip(neutral_examples, neutral_labels)):
        idx_l = list(range(len(examples)))
        random.shuffle(idx_l)
        examples = [examples[idx] for idx in idx_l]
        labels = [labels[idx] for idx in idx_l]
        data['train']['example'][f'neutral{i}'] = examples[args.dev_data_size:10000]
        data['train']['label'][f'neutral{i}'] = labels[args.dev_data_size:10000]
        data['dev']['example'][f'neutral{i}'] = examples[:args.dev_data_size]
        data['dev']['label'][f'neutral{i}'] = labels[:args.dev_data_size]
    return data
...
def create_dataloader(args, datasets, tokenizer, train=False):
    ...
    max_size = max([len(value) for key, value in datasets.items() if 'neutral' not in key])
    print("max size"+str(max_size))
    min_size = min([len(value) for key, value in datasets.items() if 'neutral' not in key])
...
def train(args, data, datasets, model: PreTrainedModel, original_model, tokenizer: PreTrainedTokenizer) -> Tuple[int, float]:
    ...
    def attribute_vector_example():
        ...
        for i in range(attribute_size - 2):
...
def main():
    ...
    if args.tokenizer_name:
        ...
        tokenizer = AutoTokenizer.from_pretrained(args.model_name_or_path, cache_dir=args.cache_dir,model_max_length=512)
    else:
        raise ValueError(
